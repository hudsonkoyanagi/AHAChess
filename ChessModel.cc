...
MOVE_RESULTS ChessModel::check_pre_move(Move m, bool white_to_move) {
  ...
  case KING: {
    ...
    if (abs(m.end.row - m.start.row) == 0 && abs(m.end.col - m.start.col) == 2) {
      ...
      if (p->col == WHITE) {
        row = 1;
        if (m.start.col > m.end.col) should_be_rook = at("a1");
        if (m.start.col < m.end.col) should_be_rook = at("h1");
      } else { // BLACK
        row = 8;
        if (m.start.col > m.end.col) should_be_rook = at("a8");
        if (m.start.col < m.end.col) should_be_rook = at("h8");
      } 
      if(should_be_rook == nullptr) return INVALID_MOVE;
