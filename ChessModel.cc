...
ATTEMPT_RESULT ChessModel::attempt_move(Cord start, Cord end, bool white_to_move, bool isBot) {
  ...
  if ((white_to_move && is_white_temp_in_check) || (!white_to_move && is_black_temp_in_check)) { // put yourself in check
  ...
  for (int r = 0; r < 8; ++r) {
    for (int c = 0; c < 8; ++c) {
      if (b[r][c]->col == BLACK) {
        ...
        for (auto cord : moves) {
          if (cord.row == k_loc.row && cord.col == k_loc.col) // king is under attack 
          {
            return true;
          }
        }
  ...
  for (int r = 0; r < 8; ++r) {
    for (int c = 0; c < 8; ++c) {
      if (b[r][c]->col == WHITE) {
        ...
        for (auto cord : moves) {
          if (cord.row == k_loc.row && cord.col == k_loc.col) //  king is under attack after
          {
            return true;
          }
        }
...
bool ChessModel::is_valid_king_move(Cord start, Cord end) {
  ...
  if (abs(end.row - start.row) == 0 && abs(end.col - start.col) == 2) {
    ...
    if (board[start.row][start.col]->col == WHITE) {
      if (start.col > end.col) { // Queen side castle
        if (!(at("b1")->is_empty()) || !(at("c1")->is_empty()) || !(at("d1")->is_empty())) return false;
        ...
        for (int r = 0;r < 8;r++) {
          for (int c = 0;c < 8;c++) {
            if (board[r][c]->col == BLACK) {
              if (board[r][c]->type == KING && r == 0) continue;
              std::vector<Cord> valid_ends = get_all_valid_end_cords(Cord{ r,c }, false);
              for (auto c : valid_ends) if (c.row == 7 && c.col == 3) return false; //d1
            }
          }
        }
      } else { // King side castle
        if (!(at("f1")->is_empty()) || !(at("g1")->is_empty())) return false;
        for (int r = 0;r < 8;r++) {
          for (int c = 0;c < 8;c++) {
            if (board[r][c]->col == BLACK) {
              if (board[r][c]->type == KING && r == 0) continue;
              std::vector<Cord> valid_ends = get_all_valid_end_cords(Cord{ r,c }, false);
              for (auto c : valid_ends) if (c.row == 7 && c.col == 5) return false; //f1
            }
          }
        }
      }
      ...
      if (start.col > end.col) { // Queen side castle
        if (!(at("b8")->is_empty()) || !(at("c8")->is_empty()) || !(at("d8")->is_empty())) return false;
        for (int r = 0;r < 8;r++) {
          for (int c = 0;c < 8;c++) {
            if (board[r][c]->col == WHITE) {
              if (board[r][c]->type == KING && r == 7) continue;
              std::vector<Cord> valid_ends = get_all_valid_end_cords(Cord{ r,c }, true);
              for (auto c : valid_ends) if (c.row == 0 && c.col == 3) return false; //d8
            }
          }
        }
      } else { // King side castle
        if (!(at("f8")->is_empty()) || !(at("g8")->is_empty())) return false;
        for (int r = 0;r < 8;r++) {
          for (int c = 0;c < 8;c++) {
            if (board[r][c]->col == WHITE) {
              if (board[r][c]->type == KING && r == 7) continue;
              std::vector<Cord> valid_ends = get_all_valid_end_cords(Cord{ r,c }, true);
              for (auto c : valid_ends) if (c.row == 0 && c.col == 5) return false; // f8
            }
          }
        }
      }
...
bool ChessModel::is_valid_pawn_move(Cord start, Cord end) {
  if (start.col != end.col) { // capture/en pesant
    ...
    if (board[end.row][end.col]->type != EMPTY)
      return true; // must capture a piece
    ...
    if (board[end.row][end.col]->type == EMPTY) { // en passant
      ...
      if (other_pawn->type == PAWN && other_pawn->col != board[start.row][start.col]->col) {
        ...
        if (last_move.moved != PAWN) return false;
        if (abs(last_move.start.row - last_move.end.row) != 2) return false;
        if (last_move.start.col != last_move.end.col || last_move.start.col != end.col) return false;
        return true;
      }
      return false;
    }
  ...
  if (abs(start.row - end.row) == 2) { // starting square
    if ((board[start.row][start.col]->col == WHITE) && (start.row != 6) && (end.row != 4)) return false; // must be on starting square
    if ((board[start.row][start.col]->col == BLACK) && (start.row != 1) && (end.row != 3)) return false; // must be on starting square
    if (board[end.row][end.col]->type != EMPTY) return false; // blocked
    if ((board[start.row][start.col]->col == WHITE) && (board[5][start.col]->type != EMPTY)) return false; // blocked
    if ((board[start.row][start.col]->col == BLACK) && (board[2][start.col]->type != EMPTY)) return false; // blocked
    return true;
  }
  if (abs(start.row - end.row) == 1) { // forward move, possibly promotion
    if ((board[start.row][start.col]->col == WHITE) && (start.row <= end.row)) return false;
    if ((board[start.row][start.col]->col == BLACK) && (start.row >= end.row)) return false;
    if (board[end.row][end.col]->type != EMPTY) return false;
    return true;
  }
  return false;
}
...
bool ChessModel::is_valid_pawn_move(Cord start, Cord end, std::array<std::array<Piece*, 8>, 8>& b) {
  if (start.col != end.col) { // capture/en pesant
    if (abs(start.row - end.row) != 1) return false; // move one forward
    if (abs(start.col - end.col) != 1) return false; // move one sideways
    if ((b[start.row][start.col]->col == WHITE) && (start.row <= end.row)) return false; // backwards move
    if ((b[start.row][start.col]->col == BLACK) && (start.row >= end.row)) return false; // backwards move
    if (b[end.row][end.col]->col == board[start.row][start.col]->col) return false; // same colour capture
    if (b[end.row][end.col]->type == KING) return false; // can't capture king (TODO game should be over here anyway)
    if (b[end.row][end.col]->type != EMPTY) return true;
  ...
  if (abs(start.row - end.row) == 2) { // starting square
    if ((b[start.row][start.col]->col == WHITE) && (start.row != 6) && (end.row != 4)) return false; // must be on starting square
    if ((b[start.row][start.col]->col == BLACK) && (start.row != 1) && (end.row != 3)) return false; // must be on starting square
    if (b[end.row][end.col]->type != EMPTY) return false; // blocked
    if ((b[start.row][start.col]->col == WHITE) && (b[5][start.col]->type != EMPTY)) return false; // blocked
    if ((b[start.row][start.col]->col == BLACK) && (b[2][start.col]->type != EMPTY)) return false; // blocked
    return true;
  }

  if (abs(start.row - end.row) == 1) { // forward move, possibly promotion
    if ((b[start.row][start.col]->col == WHITE) && (start.row <= end.row)) return false;
    if ((b[start.row][start.col]->col == BLACK) && (start.row >= end.row)) return false;
    if (b[end.row][end.col]->type != EMPTY) return false;
    return true;
  }
  return false;
}
...
std::pair<ATTEMPT_RESULT, Move> ChessModel::attempt_move_no_commit(Cord start, Cord end, bool white_to_move) {
  ...
  if (white_to_move && p->col != WHITE) return std::make_pair(ret, mv);
  if (!white_to_move && p->col != BLACK) return std::make_pair(ret, mv);
  ...
  if (!found) return std::make_pair(ret, mv);
  // Move is semantically legal here
  std::array<std::array<Piece*, 8>, 8> temp_board = boardCopy();
