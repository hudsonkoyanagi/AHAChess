...
ATTEMPT_RESULT ChessModel::attempt_move(Cord start, Cord end, bool white_to_move) {
    ...
    // for all valid end cords:
    //      if not in: invalid move, return
    //  (move is semantically legal)
    // temp_board.make_move(start, end)
    // 
    // if (start col == white)
    // if(temp_board.is_white_in_check()) invalid
    //      if(temp_board.is_black_in_check()) commit move and return in check, and set white not in check
    //      is valid so commit to board and return and set white not in check
    // (same for black)
}
...
std::vector<Cord> ChessModel::get_all_valid_end_cords(Cord start, bool white_to_move) {
    ...
    for(int r = 0; r < 8; ++r) {
        for(int c = 0; c < 8; ++c) {
            switch(p->type) {
                case KNIGHT:
                    if(is_valid_knight_move(start, Cord{r,c})) moves.push_back(Cord{r,c});
                    break;
            }
        }
    }
...
bool ChessModel::is_valid_knight_move(Cord start, Cord end) {
    ...
    if (target->col == p->col) return false; 
